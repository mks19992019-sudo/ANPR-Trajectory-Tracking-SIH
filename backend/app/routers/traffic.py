...
def get_live_traffic_volume(db: Session = Depends(get_db)):
    """
    Returns real city-wide traffic volume, active cameras, average flow speed, and congested corridors.
    """
...
def get_speed_analytics(
    ...
    """
    Returns real speed percentiles (mean, median, 85th percentile, max) and compliance rates across corridors.
    """
...
def get_origin_destination_matrix(
    ...
    """
    Returns dynamic Origin-Destination (OD) trip matrix computed from vehicle entry and exit checkpoints.
    """
    return TrafficService.get_od_matrix(db, hours=hours)
